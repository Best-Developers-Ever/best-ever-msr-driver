...
fn get_windows_kits_dir() -> Result<PathBuf, Error> {
    ...
    let dir: String = hkey_local_machine.open_subkey(key)?.get_value("KitsRoot10")?;

    Ok(dir.into())
}

fn get_km_dir(windows_kits_dir: &PathBuf) -> Result<PathBuf, Error> {
    let read_dir = Path::new(windows_kits_dir).join("lib").read_dir()?;

    let max_lib_dir = read_dir
